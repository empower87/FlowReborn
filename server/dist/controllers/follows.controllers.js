...
export const followHandler = async ({ ctx, input }) => {
    if (!ctx.user)
    ...
    if (ctx.user.username === user.username)
        throw TRPCError("BAD_REQUEST", "you can not follow yourself");
...
export const unfollowHandler = async ({ ctx, input }) => {
    if (!ctx.user)
        throw TRPCError("UNAUTHORIZED", "user not authorized to comment");
    const { follower, following } = input;
    const me = await User.findById(follower);
    ...
    if (!me || !user)
        throw TRPCError("INTERNAL_SERVER_ERROR", "user not found");
    if (ctx.user.username === user.username)
        throw TRPCError("BAD_REQUEST", "you can not unfollow yourself");
    if (!me.following.includes(following) || !user.followers.includes(follower))
        throw TRPCError("BAD_REQUEST", "you already following this user");
...
//# sourceMappingURL=follows.controllers.js.map